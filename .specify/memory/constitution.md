<!--
Sync Impact Report
- Version change: (scaffold Spec Kit sem versão) → 1.0.0
- Modified principles: placeholders genéricos ([PRINCIPLE_1_NAME]…[PRINCIPLE_5_NAME]) → sete Core Principles da base IA-dev Labs v1.0.0 (I–VII), sem redefinição incompatível
- Added sections: declaração da Base de engenharia e Contexto do projeto; Princípios e restrições específicos do projeto; Restrições técnicas e de segurança; Fluxo de desenvolvimento e critérios de qualidade (incluindo Spec-Driven Development, critérios de qualidade e estratégia obrigatória de testes); Governance alinhada à base IA-dev Labs
- Removed sections: seções placeholder [SECTION_2_NAME] e [SECTION_3_NAME] do scaffold genérico
- Follow-up TODOs: nenhum placeholder intencionalmente diferido
- Divergências da base global: nenhuma
-->

# teste-preset Constitution
...
**Contexto do projeto**: aplicação web; esta Constitution registra os requisitos mínimos de engenharia aplicáveis a esse tipo de sistema, sem fixar stack, produto ou estratégia de testes além da baseline.
...
Nenhum guardrail específico do projeto foi estabelecido neste momento.
...
- Contratos externos relevantes DEVEM se basear em documentação, schemas, APIs ou outras fontes autoritativas vigentes.
- Dados históricos que representem fatos já ocorridos NÃO DEVEM ser reescritos silenciosamente quando isso comprometer auditoria, rastreabilidade ou correção do domínio.
- Áreas privadas ou administrativas DEVEM possuir proteção real de autenticação e autorização quando esses conceitos forem aplicáveis.

Nenhuma restrição técnica específica do projeto foi estabelecida neste momento além da baseline acima.
...
- **Domínio/unidade**: usar quando a regra puder ser verificada sem banco, browser, rede ou infraestrutura externa.
- **Integração**: usar quando a garantia depender da interação real entre persistência, autorização, constraints, transações, migrations, filas, caches ou infraestrutura equivalente.
- **Componente/UI**: quando houver interface, usar quando for o nível mais simples para validar comportamento visível sem executar o fluxo completo.
- **E2E**: usar seletivamente para fluxos de alto valor ou risco que atravessem múltiplas fronteiras reais e não possam ser suficientemente provados por níveis inferiores.
- **Contrato/integrações externas**: verificar localmente serialização, parsing, validação, mapeamento de erros e contratos sempre que aplicável.
