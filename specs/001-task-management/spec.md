...
Como usuário, quero criar tarefas e consultar a lista de tarefas existentes para registrar e acompanhar o que preciso fazer.

**Why this priority**: Registrar e reencontrar tarefas é o valor central da aplicação e habilita os demais fluxos.

**Independent Test**: Criar uma tarefa, sair da aplicação, retornar e verificar que a tarefa continua disponível.
...
3. **Given** que uma tarefa foi criada, **When** o usuário encerra e depois acessa novamente a aplicação, **Then** a tarefa permanece disponível com seus dados e estado anteriores.
...
- **FR-007**: O sistema DEVE preservar as tarefas criadas e seus estados para o acesso identificado do usuário, permitindo recuperá-las em diferentes dispositivos.
...
| **CB-003** | Exclusão da tarefa solicitada | Uma tarefa incorreta pode ser removida ou permanecer indevidamente na lista | Somente a tarefa escolhida deixa de ser apresentada após exclusão confirmada |
...
- **SC-003**: Em uma avaliação funcional, 100% das exclusões confirmadas removem a tarefa selecionada da lista sem remover tarefas não selecionadas.
